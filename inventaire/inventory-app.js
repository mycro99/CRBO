...
import { articleFrom, categoryLabel, fold, normalizeBarcode, barcodeMatches, quantity, validDate, todayISO, expiryState, formatDate, safeImage, unallocatedStock, expiries, primaryExpiry, orderNeed, matchesFilter, splitBarcodes } from './inventory-core.js';
...
let submitHandler=null, formSaving=false, formDirty=false, formOperationId='', toastTimer;
...
function updateStatus(){
  ...
  $('editor-submit').disabled=Boolean(submitHandler) && !writable();
...
function render(){
  ...
  for(const key of ['all','zero','low','expiry','ordered'])$('stat-'+key).textContent=state.articles.filter(a=>matchesFilter(a,key)).length;
...
function row(a){
  const image=safeImage(a.image), expiry=primaryExpiry(a), draft=countDraft.get(a.id);
  ...
  const stock=countMode&&a.active?`${a.lots.length?'<span class="muted">Comptage par lot</span>':`<input class="count-input" data-count-id="${e(a.id)}" type="number" min="0" max="9999999" step="1" inputmode="numeric" placeholder="Compté" value="${e(draft?.value??'')}" aria-label="Quantité comptée : ${e(a.name)}" ${a.stockValid?'':'disabled'}>`}<small class="article-meta">Enregistré : ${a.stockValid?a.stock:'À vérifier'}</small>`:
    `<div class="stock-controls"><button type="button" data-action="minus" data-id="${e(a.id)}" aria-label="Retirer une unité : ${e(a.name)}" ${disabled}>−</button><span class="stock-value ${stockClass}">${a.stockValid?a.stock:'?'}</span><button type="button" data-action="plus" data-id="${e(a.id)}" aria-label="Ajouter une unité : ${e(a.name)}" ${disabled}>+</button></div><span class="article-meta">${e(a.unit)}</span>`;
...
function openDialog(title,body,onSubmit=null,submitLabel='Enregistrer'){
  ...
  submitHandler=onSubmit;formOperationId=uid();formDirty=false;
...
$('editor-form').addEventListener('submit',async event=>{
  event.preventDefault();if(!submitHandler||formSaving)return;
...
function fillCategoryDatalist(){return `<datalist id="category-options">${categories().map(c=>`<option value="${e(categoryLabel(c))}"></option>`).join('')}</datalist>`;}
...
function editArticle(a=null,barcode=''){
  ...
  const body=`<p class="muted">${a?'La fiche ne modifie ni le stock ni les péremptions.':'L’article sera disponible dans l’inventaire et le scanner après enregistrement.'}</p><div class="form-grid">
    ...
    ${input('category','Catégorie',a?categoryLabel(a.category):'', 'text','required maxlength="80" list="category-options"')}${input('location','Emplacement',a?.location||'','text','maxlength="120"')}
    ...
    <label class="full">Photo ou lien HTTPS (facultatif)<input id="article-image" name="image" value="${e(safeImage(a?.image))}" maxlength="1500" placeholder="Image existante ou lien https://…"><small>Sans photo, une vignette neutre est affichée. Aucun service d’hébergement payant n’est ajouté.</small></label>
    <label class="full">Notes<textarea name="notes" rows="2" maxlength="1000">${e(a?.notes||'')}</textarea></label>
    </div>${fillCategoryDatalist()}${!a?`<h3>Péremption du stock initial</h3>${dateFields({expirationDate:'',noExpiration:false})}`:''}`;
  openDialog(a?'Modifier — '+a.name:'Nouvel article',body,async(fd,opId)=>{
    const catalog=Object.fromEntries(fd);catalog.category=resolveCategory(catalog.category);catalog.barcodes=splitBarcodes(catalog.barcodes);
  ...
  $('photo-picker').addEventListener('change',event=>{if(event.target.value){$('article-image').value=event.target.value==='none'?'':event.target.value;formDirty=true;}});
...
document.addEventListener('click',event=>{
  ...
  const actions={detail:()=>showDetail(a),edit:()=>editArticle(a),move:()=>movement(a),plus:()=>quickMove(a,1),minus:()=>quickMove(a,-1),expiry:()=>editExpiry(a),lotExpiry:()=>editExpiry(a,b.dataset.lotId),lot:()=>addLot(a),archive:()=>archive(a),order:()=>order(a),receive:()=>receive(a)};
...
$('count-mode').addEventListener('click',()=>{countMode=true;filter='all';$('filter').value=filter;saveCountDraft();render();$('count-panel').scrollIntoView({behavior:'smooth',block:'center'});});
...
window.addEventListener('beforeunload',event=>{if(busy||formDirty){event.preventDefault();event.returnValue='';}});
...
async function start(){
  try{
    const client=await import('./firebase-client.js');store=client.store;
    store.subscribe(next=>{state=next;render();initialNavigation();});
    client.watchAuth(user=>{
      unsubscribeInventory();unsubscribeLast();authUser=user;
